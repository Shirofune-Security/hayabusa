...
- Renamed the obscure `afterfact` module and its identifiers to self-describing result-output names: `src/afterfact.rs`→`src/results.rs`, `AfterfactInfo`→`ResultOutputState`, `AfterfactWriter`→`OutputWriter`, `output_afterfact`→`output_results`, `output_additional_afterfact`→`output_result_summary`, and the local `afterfact_info`/`afterfact_writer`→`result_state`/`output_writer`. Pure rename; no behavior change (full test suite passes). (#1853) (@YamatoSecurity)
